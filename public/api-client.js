...
(function () {
  ...
  const LIVE = {
    ...
    admin: {
      ...
      getBreweryStyles:     (id) => req(`/admin/breweries/${encodeURIComponent(id)}/styles`),
      setBreweryStyles:     (id, styles) => req(`/admin/breweries/${encodeURIComponent(id)}/styles`, { method: "PUT", body: { styles } }),
      ...
      validateResetToken:   (token) => req(`/admin/validate-reset-token?token=${encodeURIComponent(token)}`),
      listStyles:           () => req("/admin/styles"),
      createStyle:          (data) => req("/admin/styles", { method: "POST", body: data }),
      updateStyle:          (id, data) => req(`/admin/styles/${encodeURIComponent(id)}`, { method: "PUT", body: data }),
      deleteStyle:          (id) => req(`/admin/styles/${encodeURIComponent(id)}`, { method: "DELETE" }),
      listGlossary:         () => req("/admin/glossary"),
      createGlossaryEntry:  (data) => req("/admin/glossary", { method: "POST", body: data }),
      updateGlossaryEntry:  (term, data) => req(`/admin/glossary/${encodeURIComponent(term)}`, { method: "PUT", body: data }),
      deleteGlossaryEntry:  (term) => req(`/admin/glossary/${encodeURIComponent(term)}`, { method: "DELETE" }),
      listPrices:           (breweryId) => req(`/admin/prices${breweryId ? "?breweryId=" + encodeURIComponent(breweryId) : ""}`),
      createPrice:          (data) => req("/admin/prices", { method: "POST", body: data }),
      updatePrice:          (id, data) => req(`/admin/prices/${encodeURIComponent(id)}`, { method: "PUT", body: data }),
      ...
      updateEvent:          (id, data) => req(`/admin/events/${encodeURIComponent(id)}`, { method: "PUT", body: data }),
  ...
  window.AtlasAPI = {
    ...
    admin: {
      ...
      // validateResetToken und requestReset/resetPassword brauchen keinen Auth-Gate
      validateResetToken: (token) => LIVE.admin.validateResetToken(token),
      requestReset:       (email) => LIVE.admin.requestReset(email),
      resetPassword:      (token, password) => LIVE.admin.resetPassword(token, password),
      ...
      getBreweryStyles:   (id) => window.AtlasAPI.admin._gate("getBreweryStyles", id),
      setBreweryStyles:   (id, styles) => window.AtlasAPI.admin._gate("setBreweryStyles", id, styles),
      listStyles:         () => window.AtlasAPI.admin._gate("listStyles"),
      createStyle:        (data) => window.AtlasAPI.admin._gate("createStyle", data),
      updateStyle:        (id, data) => window.AtlasAPI.admin._gate("updateStyle", id, data),
      deleteStyle:        (id) => window.AtlasAPI.admin._gate("deleteStyle", id),
      listGlossary:       () => window.AtlasAPI.admin._gate("listGlossary"),
      createGlossaryEntry:(data) => window.AtlasAPI.admin._gate("createGlossaryEntry", data),
      updateGlossaryEntry:(term, data) => window.AtlasAPI.admin._gate("updateGlossaryEntry", term, data),
      deleteGlossaryEntry:(term) => window.AtlasAPI.admin._gate("deleteGlossaryEntry", term),
      listPrices:         (breweryId) => window.AtlasAPI.admin._gate("listPrices", breweryId),
      createPrice:        (data) => window.AtlasAPI.admin._gate("createPrice", data),
      updatePrice:        (id, data) => window.AtlasAPI.admin._gate("updatePrice", id, data),
      deletePrice:        (id) => window.AtlasAPI.admin._gate("deletePrice", id),
      listEvents:         () => window.AtlasAPI.admin._gate("listEvents"),
      createEvent:        (data) => window.AtlasAPI.admin._gate("createEvent", data),
      updateEvent:        (id, data) => window.AtlasAPI.admin._gate("updateEvent", id, data),
